...
### 2.4 协议与兼容性（OTLP / Langfuse v3+ / LiteFuse）

WeKnora 上报走 **OTLP/HTTP**（`POST /api/public/otel/v1/traces`，`ExportTraceServiceRequest` JSON），这是 Langfuse v3+ 的标准 ingestion 协议。旧版 `/api/public/ingestion` 批量 API（`trace-create`/`span-create`/`generation-create` 事件）已被弃用且不再发送。

- **后端要求**：需 **Langfuse v3+** 或 **LiteFuse**（Langfuse 的 Doris 后端 fork，存储成本更低、无需维护 ClickHouse）。仅支持旧版 ingestion 的 Langfuse v2 实例不可用。
- **OTel ingestion gate**：Langfuse v3+ / LiteFuse 的 OTel 端点要求现代 SDK 标识才放行（否则返回 400 `requires Python SDK >= 4.0.0 or JS SDK >= 5.0.0`）。WeKnora 发 `x-langfuse-ingestion-version: 4` header（OTel exporter direct-write opt-in）通过门槛；`x-langfuse-sdk-name: python` 仅为兼容该 gate 的标记——**WeKnora 是 Go 自实现客户端，并非真的 python SDK**，运维看到该 header 时不必困惑。
- **自建 LiteFuse / 内网**：`LANGFUSE_HOST` 指向 LiteFuse web 服务地址。k8s 集群内优先用 service DNS（如 `http://litefuse-web.<ns>.svc.cluster.local:3000`），不要走 Ingress 域名——Pod 内 CoreDNS 可能解析不了外部 Ingress 主机名。
